...
def detect_signal(df: pd.DataFrame, ticker: str) -> str:
    ...
    # ── SELL: MACD cruza abajo de signal ──────────────────
    if macd_p >= macd_sig_p and macd < macd_sig:
        logger.info(
            f"[MACD_BB] {ticker} SELL ✅ — MACD cruzó bajo signal | "
            f"macd={macd:.4f} sig={macd_sig:.4f}"
        )
        return "SELL"

    # ── BUY: BB expandiendo + close cruza BB_upper + MACD alcista ──
    ...
    if bb_expanding and price_breakout and bullish_candle and macd_bullish:
        logger.info(
            f"[MACD_BB] {ticker} BUY ✅ — BB breakout + MACD bullish | "
