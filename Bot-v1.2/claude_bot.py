# ============================================================
#  EOLO v1.2 — Claude Bot (Estrategia #14)
#
#  Motor de decisión autónomo basado en la API de Anthropic.
#  Corre en paralelo con las 13 estrategias técnicas. Cada 60s:
...
#          "price": ..., "reasoning": ..., "candle_time": ...}
#    4. Registra cada decisión en Firestore
#       (eolo-claude-decisions-v12/{YYYY-MM-DD}/decisions/{ts}).
#
...
def _load_api_key_from_secret_manager(project_id: str, secret_name: str = "ANTHROPIC_API_KEY") -> str | None:
    ...
    try:
        ...
        logger.warning(f"[CLAUDE_BOT] No pude leer {secret_name} de Secret Manager: {e}")
        return None
...
class ClaudeBotEngine:
    """
    Motor autónomo de decisión. Safe by default: en paper mode
    (flag `paper_mode=True`) solo registra decisiones, no ejecuta.
    """
    ...
    async def decide(self, snapshot: dict) -> dict:
        ...
        try:
            ...
            logger.error(f"[CLAUDE_BOT] Error llamando a Claude: {e}\n{traceback.format_exc()}")
            return {
    ...
    def _build_prompt(self, snap: dict) -> str:
        ...
        prompt = f"""Sos el motor de decisión autónomo "Claude Bot" dentro del sistema EOLO v1.2, un bot de day-trading de acciones en US equities.

Tu rol: sos la **estrategia #14** que corre en paralelo con 13 estrategias técnicas tradicionales (EMA, gap fade, RSI, Bollinger, VWAP, supertrend, etc). Tu operación es **independiente**: tomás decisiones basadas en el snapshot actual del mercado y opcionalmente considerás qué están diciendo las otras estrategias como pista, pero no tenés que validarlas.
