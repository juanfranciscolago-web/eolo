...
POSITIONS_COLLECTION = "eolo-config-v12"
POSITIONS_DOC        = "positions"

# ── Firestore v1.2 (namespace separado de v1) ────────────
CONFIG_COLLECTION_V12 = "eolo-config-v12"
TRADES_COLLECTION_V12 = "eolo-trades-v12"
...
def get_trade_budget() -> float:
    ...
    try:
        ...
        from secret_stuff import project_id
        db  = firestore.Client(project=project_id)
        doc = db.collection(CONFIG_COLLECTION_V12).document("settings").get()
        if doc.exists:
...
def _log_trade(action: str, ticker: str, shares: int, price: float, strategy: str = ""):
    ...
    try:
        from google.cloud import firestore
        db  = firestore.Client(project=project_id)
        doc = db.collection(TRADES_COLLECTION_V12).document(today)
        doc.set({
