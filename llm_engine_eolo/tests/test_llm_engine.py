...
def test_kb_loads():
    """KB Excel v1.8 carga OK.

    v1.8: 108 reglas físicas, 107 efectivas (TR-Juan-056 deprecated_by=TR-Juan-083).
    Tier bump 091-103 TACTICAL_PLUS → MAESTRA + nueva col Deprecated_By.
    Stats vs v1.7: MAESTRA 20→33, TACTICAL_PLUS 41→28, PROHIBITIVA 9→8 (deprecated).
    """
    kb = KBLoader(KB_PATH)
    stats = kb.stats()
    ...
    for rule_num in range(48, 62):
        if rule_num == 56:
            continue  # v1.8: TR-Juan-056 deprecated_by=TR-Juan-083
        rule = kb.get_rule_by_id(f"TR-Juan-{rule_num:03d}")
